...
int main(int argc, char** argv)
{
    ...
    h_A = (float*)malloc(sizeof(float) * HA * WA);
    h_B = (float*)malloc(sizeof(float) * HB * WB);
    h_C = (float*)malloc(sizeof(float) * HC * WC);
    ...
    d_C = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float)*HC*WC, NULL, &err);
    d_A = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(float)*HA*WA, h_A, &err);
    d_B = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(float)*HB*WB, h_B, &err);
    if (!d_A || !d_B || !d_C){
    ...
    err |= clSetKernelArg(kernel, 6, sizeof(int), (void*)&hc);
    if (err != CL_SUCCESS){
    ...
    int dimension = 2;
    size_t localWorkSize[dimension], globalWorkSize[dimension];
    int local = 1;
    int global = WA >= HA ? (WA >= WB ? WA: WB) : (HA >= WB ? HA: WB);
    //Max Local is 16 apparently.
    for(int i = 16; i>1; i--){
      if (WA%i == 0 && WA/2 >= i){
        local = i;
        break;
    ...
    for(int i = 0; i < dimension; i ++){
      localWorkSize[i] = local;
      globalWorkSize[i] = global;    
    }
    ...
    err = clEnqueueNDRangeKernel(commands, kernel, dimension, NULL, globalWorkSize, localWorkSize, 0, NULL, &event);
